...
/* USER CODE BEGIN PV */
#define ADC_DATA_LENGTH  4096   //定义采集数据长度为1024
uint32_t g_adc1_dma_data1[ADC_DATA_LENGTH];//定义adc1采集数据存放数组
float32_t fftInBuf[2*ADC_DATA_LENGTH];
//float32_t fftOutBuf[ADC_DATA_LENGTH];
float32_t fftMagBuf[ADC_DATA_LENGTH];
...
//				{
//					fftInBuf[2*i]=(float)(g_adc1_dma_data1[i]&0x0000ffff)/65536*3.3f;
//					fftInBuf[(2*i)+1]=0;
//					printf("%2.4f V \r\n",(float)(g_adc1_dma_data1[i]&0x0000ffff)/65536*3.3);
//					printf("%2.4f V \r\n",(float)(g_adc1_dma_data1[i]&0x0000ffff)/65536*3.3);
//					//printf("%2.4f V \r\n",(float)(g_adc1_dma_data1[i]&0x0000ffff)/65536*3.3);//采集到的值传到上位机串口，注意用仿真器的串口容易死机。
//				}
//			}
    /* USER CODE END WHILE */
	}
